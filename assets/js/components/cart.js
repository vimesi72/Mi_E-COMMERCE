function cart(db, printProducts, findProduct) {
  ...
  function addToCart(id, qty = 1) {
    ...
    const itemFinded = index >= 0 ? cart[index] : null;
    
    if (itemFinded) {
      const stock = db.find((item) => item.id === id);
      console.log((stock.quantity - (itemFinded.qty + 1)) >= 0)
      if ( verifyStock(id, qty + itemFinded.qty)) {
        itemFinded.qty += qty
      } else { window.alert("Producto sin Stock") }
  ...
  function verifyStock(id, quantity) {
    return findProduct(id).quantity - quantity >= 0;
  }
